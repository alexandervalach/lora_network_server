...
public class EDProcessor {
  ...
  public void processRXL(ArrayList<JSONObject> currentGrape) {
    try {
      ...
      if (messageBody != null) {
        ...
        try {
          remainingDutyC = this.getRemainingDutyCycle(messageBody, primary.getInt("sf"), primary.getInt("duty_c"));
        } catch (Exception e) {
   ...
   * @param spf used spreading factor
   * @return int
   * @throws Exception
   */
  ...
  private JSONObject adrAlgorithm (JSONObject primary, int finalRssi, int finalSnr) throws JSONException {
    ...
    if ((ackType.equals("VOLATILE") || ackType.equals("MANDATORY")) && (!rawResponse.toString().equals("{}") || confNeed || ackType.equals("MANDATORY") || powerChanged)) {
      ...
      try {
        messageBody.put("time", this.getMsgCost(primary, spf));
      } catch (Exception e) {
